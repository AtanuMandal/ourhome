    ...
    : IRequestHandler<AcceptServiceRequestCommand, Result<bool>>
{
    public async Task<Result<bool>> Handle(AcceptServiceRequestCommand request, CancellationToken ct)
    {
        try
        {
            ...
            if (provider is null)
                return Result<bool>.Failure(ErrorCodes.ServiceProviderNotFound, $"Service provider with id '{request.ProviderId}' was not found.");
            ...
            if (serviceRequest is null)
                return Result<bool>.Failure(ErrorCodes.ServiceRequestNotFound, $"Service request with id '{request.RequestId}' was not found.");
