...
export function VendorPaymentListScreen() {
  ...
  function handleMarkPaid(item: VendorCharge): void {
    Alert.alert('Mark paid', `Mark the ₹${item.amt} charge to ${item.vnm} as paid?`, [
  ...
  function handleDelete(item: VendorCharge): void {
    Alert.alert('Delete charge', `Delete the ${item.ct} charge to ${item.vnm}? This cannot be undone.`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete',
        style: 'destructive',
  ...
  function renderItem({ item }: { item: VendorCharge }) {
    ...
    return (
      <View style={[styles.item, !item.ac && styles.itemInactive]}>
        <View style={styles.itemTop}>
          <Text style={styles.vendor}>{item.vnm}</Text>
          <CurrencyText amount={item.amt} style={styles.amount} />
        </View>
        <Text style={styles.category}>{item.ct}{!item.ac ? ' • Inactive' : ''}</Text>
        <Text style={styles.description} numberOfLines={1}>{item.ds}</Text>
        <View style={styles.itemBottom}>
          <StatusChip status={item.st} />
          <Text style={styles.date}>{formatDate(item.dd)}</Text>
        </View>
        {isAdmin && (
          <View style={styles.actionRow}>
            {item.st !== 'Paid' && item.ac && (
            ...
            {item.st !== 'Paid' && (
              <TouchableOpacity
                ...
                onPress={() => setActive.mutate(
                  { chargeId: item.id, active: !item.ac },
                  { onError: (e) => Alert.alert('Could not update charge', normalizeError(e)) }
                )}
              >
                <Text style={styles.actionBtnText}>{item.ac ? 'Inactivate' : 'Activate'}</Text>
              </TouchableOpacity>
            )}
            {item.st !== 'Paid' && (
