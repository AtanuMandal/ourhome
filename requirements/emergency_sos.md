...
> ✅ **Implemented.** The `SosAlert` entity, all endpoints below, and web + mobile UI (trigger card, alert list with filters, admin report) are live.
...
- Marking an already-stood-down alert as `FalseAlarm` again is **idempotent** — a repeat tap (e.g. from a client showing a stale status) succeeds rather than erroring; attempting it on a `Resolved` alert returns `SOS_ALERT_ALREADY_SETTLED` (HTTP 409).
- The resident's trigger card polls the alert status live until it settles, showing who acknowledged/resolved it.
...
| `GET` | `/api/societies/{id}/sos-alerts` | Authenticated | List alerts (active + history) with status/category/date filters |
